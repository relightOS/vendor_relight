...
cat <<EOF
Additional Radiant functions:
- cout:            Changes directory to out.
- mmp:             Builds all of the modules in the current directory and pushes them to the device.
- mmap:            Builds all of the modules in the current directory and its dependencies, then pushes the package to the device.
- mmmp:            Builds all of the modules in the supplied directories and pushes them to the device.
- radiantgerrit:   A Git wrapper that fetches/pushes patch from/to Radiant Gerrit Review.
- radiantrebase:   Rebase a Gerrit change and push it again.
- radiantremote:   Add git remote for Radiant Gerrit Review.
- aospremote:      Add git remote for matching AOSP repository.
- cafremote:       Add git remote for matching CodeAurora repository.
- githubremote:    Add git remote for Radiant Github.
...
function radiantremote()
{
    ...
    local RADIANT_USER=$(git config --get review.review.ProjectRadiant.org.username)
    if [ -z "$RADIANT_USER" ]
    then
        git remote add radiant ssh://review.ProjectRadiant.org:29418/$PFX$PROJECT
    else
        git remote add radiant ssh://$RADIANT_USER@review.ProjectRadiant.org:29418/$PFX$PROJECT
    fi
...
function githubremote()
{
    ...
    git remote add github https://github.com/ProjectRadiant/$PROJECT
...
function radiantgerrit() {
    ...
    local user=`git config --get review.review.ProjectRadiant.org.username`
...
function radiantrebase() {
    ...
    if [ -z $repo ] || [ -z $refs ]; then
        echo "Radiant Gerrit Rebase Usage: "
    ...
    git fetch "http://review.ProjectRadiant.org/p/$repo" "refs/changes/$refs" && git cherry-pick FETCH_HEAD
    if [ "$?" != "0" ]; then
...
function repopick() {
    T=$(gettop)
    $T/vendor/relight/build/tools/repopick.py $@
}
