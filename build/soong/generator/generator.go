...
func init() {
	android.RegisterModuleType("radiant_generator", GeneratorFactory)
...
func (g *Module) GenerateAndroidBuildActions(ctx android.ModuleContext) {
	...
	if depRoot == "" {
		...
		depRoot = radiantExpandVariables(ctx, depRoot)
	}

	// Glob dep_files property
	for _, dep_file := range g.properties.Dep_files {
		dep_file = radiantExpandVariables(ctx, dep_file)
	...
	exCmd := radiantExpandVariables(ctx, String(g.properties.Cmd))
